...
class SetCityAsFavoriteUseCaseTest {
    private lateinit var useCase: SetCityAsFavoriteUseCase
    ...
    fun `should emit Success after remove city from favorites`() =
        runTest {
            // Given
            val cityId = "1"
            coEvery { cityRepositoryMock.saveFavoriteCity(cityId) } just Runs
            ...
            // Then
            coVerify {
                cityRepositoryMock.saveFavoriteCity(cityId)
            }
    ...
    fun `should emit Error when the repository returns error`() =
        runTest {
            ...
            coEvery { cityRepositoryMock.saveFavoriteCity(cityId) } throws error
